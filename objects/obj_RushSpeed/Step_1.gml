#region //Attacking
	if (realJumping or preStomp or sliding or stomping or dropdash or dropdashing or spindash or leftTrick or rightTrick or backTrick or upTrick or homing_Active) {
		attacking = true;
	} else {
		attacking = false;
	}
...
if can_MoveFULL {
	...
	#region //Wall-Jump
		if !ground && yspd > -4 && !playerHurt && !homing_Active && !rampRing && !stomping && !preStomp && !dropdash && !spindash && !superPeelout && !instance_exists(obj_GOAL) {
			scr_WallClingStep();
		} else {
			wallJump = false;
			wallJumping = false;
